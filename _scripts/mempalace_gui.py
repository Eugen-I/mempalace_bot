...
SCRIPTS_DIR = next(
    (
        os.path.join(APP_RES, d)
        for d in ("scripts", "Scripts")
        if os.path.isdir(os.path.join(APP_RES, d))
    ),
    os.path.join(APP_RES, "scripts"),
)
FALLBACK_DIR = os.path.join(os.path.expanduser("~"), "Documents/mempalace_bot/_scripts")

USER_DIR = os.path.join(
    os.path.expanduser("~"), "Library/Application Support/MemPalaceBot",
)
...
Label(
    header,
    text="Кликните на скрипт · 🪟 Терминал  ·  ⚡ Фон",
...
def make_col(parent, title, scripts):
    ...
    Label(col, text=title, font=("Helvetica Neue", 9, "bold"), bg=BG, fg=TEXT_DIM).pack(
        anchor="w", pady=(0, 4),
    )
    ...
    for name, needs_term in scripts:
        card = Frame(
            inner, bg=CARD_BG, highlightbackground="#2a2a38", highlightthickness=1,
        )
        ...
        for w in (card, lbl, tag):
            ...
            w.bind(
                "<Enter>",
                lambda e, c=card, l=lbl, t=tag: (
                    c.configure(bg=CARD_HOVER),
                    l.configure(bg=CARD_HOVER),
...
def refresh_user_section():
    ...
    if user:
        ...
        for entry in user:
            ...
            name = (
                entry.get("name", os.path.basename(path).replace(".command", ""))
                if isinstance(entry, dict)
                else os.path.basename(path).replace(".command", "")
            )
            needs_term = (
                entry.get("terminal", True) if isinstance(entry, dict) else True
            )
...
def add_user_script():
    ...
    if not p:
        return
    needs_term = messagebox.askyesno(
        "Режим",
        "Этот скрипт требует Терминал?\n\nДа = 🪟 откроется в Терминале\nНет = ⚡ выполнится здесь",
    )
    ...
    lst.append(
        {
            "name": os.path.basename(p).replace(".command", ""),
            "path": p,
            "terminal": needs_term,
        },
    )
...
def paste_script_code():
    ...
    cb = _tk.Checkbutton(
        dialog,
        text="🪟 Открывать в Терминале (сними галочку для ⚡ фонового режима)",
        variable=term_var,
        bg=BG,
        fg=TEXT_DIM,
        selectcolor="#1a1a24",

...
from config import CHATS_DIR, allowed_callback, allowed_only
...
async def cmd_list_chats(message: types.Message):
    ...
    if not files:
        return await message.answer("📁 Чатов нет.")
    ...
    for f in files:
        ...
        kb.row(
            types.InlineKeyboardButton(
                text=f"💬 {display_name}", callback_data=f"sel:{sid}",
            ),
...
async def cb_select_chat(callback: types.CallbackQuery):
    ...
    if not fname:
        return await callback.answer(
            "❌ Сессия истекла. Откройте список чатов заново.", show_alert=True,
        )
    ...
    if len(summary) > 500 or len(messages) > 5:
        ...
        kb.row(
            types.InlineKeyboardButton(
                text="📖 Развернуть полностью", callback_data=f"show_full:{sid_full}",
            ),
        )
    ...
    await callback.message.edit_text(
        header + preview_text, parse_mode="HTML", reply_markup=kb.as_markup(),
    )
    ...
    try:
        if not files:
            ...
            for f in files:
                sid2 = _short_id(f)
                display_name = (
                    f.replace("ch_", "").replace(".json", "").replace("_", " ")[:25]
                )
                kb.row(
                    types.InlineKeyboardButton(
                        text=f"💬 {display_name}", callback_data=f"sel:{sid2}",
                    ),
                    types.InlineKeyboardButton(text="🗑️", callback_data=f"del:{sid2}"),
                )
            await callback.message.edit_text(
                "📅 Выберите чат:", reply_markup=kb.as_markup(),
            )
    except Exception:
        pass
...
async def generate_fresh_summary(messages: list, existing_summary: str = "") -> str:
    ...
    prompt = get_summary_prompt(
        dialog_slice=dialog_slice, context_prefix=context_prefix,
    )

    try:
        summary = await get_ai_response_async(
            engine, model, [{"role": "system", "content": prompt}], context="",
        )
...
async def update_summary_if_needed(user_id: int, filepath: str, messages: list):
    ...
    if len(messages) >= 5 and len(messages) % 5 == 0:
        ...
        dialog_slice = "\n".join(
            [f"{m['role']}: {m['content']}" for m in messages[-15:]],
        )
...
async def cmd_context(message: types.Message):
    ...
    if len(final_summary) > 500:
        ...
        kb.row(
            types.InlineKeyboardButton(
                text="📖 Развернуть полностью", callback_data=f"show_summary:{sid_s}",
            ),
        )
    sid_r = _short_id(fname)
    kb.row(
        types.InlineKeyboardButton(
            text="🔄 Обновить саммари", callback_data=f"refresh_summary:{sid_r}",
        ),
    )
...
async def cb_refresh_summary(callback: types.CallbackQuery):
    ...
    header = f"✅ Активен: <code>{fname}</code>\n\n📝 <b>Обновленное саммари:</b>\n{safe_html_format(preview)}"  # noqa: E501
    await callback.message.edit_text(
        header, parse_mode="HTML", reply_markup=kb.as_markup(),
    )
    cache_ai_response(
        callback.message.chat.id, callback.message.message_id, new_summary,
    )
...
async def cmd_history(message: types.Message):
    ...
    kb.row(
        types.InlineKeyboardButton(
            text="📖 Показать полностью", callback_data=f"show_full:{sid_full}",
        ),
    )

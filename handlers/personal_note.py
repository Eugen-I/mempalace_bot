...
from config import allowed_only, allowed_callback, TEMP_DIR
...
_waiting_for_note: TtlDict[int, str] = TtlDict()
_note_data: TtlDict[int, dict] = TtlDict()

FORMAT_PROMPT = """Ты — ТОЛЬКО редактор текста. Твоя задача — привести речь в читаемый вид, НЕ МЕНЯЯ СМЫСЛ.

СТРОГИЕ ПРАВИЛА:
1. Исправь пунктуацию и орфографию.
2. Удали слова-паразиты («э-э-э», «ну», «как бы», «короче», «типа», «собственно», «вообще»), стоп-слова, повторы слов.
...
5. Верни ТОЛЬКО отформатированный текст. Никаких вступлений («Вот отформатированный текст:»), пояснений, комментариев.

ПРИМЕР ЧЕГО ДЕЛАТЬ НЕЛЬЗЯ:
❌ Ввод: "ну короче вчера был стресс на работе начальник заругала"
❌ Твой ответ: "Вчера был стресс на работе: начальник ругала. Главная мысль — конфликт с руководством." ← ЭТО ИНТЕРПРЕТАЦИЯ

ПРИМЕР ЧЕГО НУЖНО ДЕЛАТЬ:
✅ Ввод: "ну короче вчера был стресс на работе начальник заругала"
✅ Твой ответ: "Вчера был стресс на работе — начальник заругала." ← ТОЛЬКО ПУНКТУАЦИЯ И УБРАНО «НУ КОРОЧЕ»

Ещё пример:
✅ Ввод: "думаю что мне стоит заняться спортом типа бегать по утрам это полезно"
✅ Твой ответ: "Думаю, что мне стоит заняться спортом — бегать по утрам. Это полезно." ← ТОЛЬКО ЗНАКИ ПРЕПИНАНИЯ"""

CLASSIFY_PROMPT = """Ниже — заметка пользователя. Определи, к какому крылу и комнате Дворца знаний её отнести.
...
async def _classify_note(formatted: str, taxonomy: dict) -> dict:
    ...
    try:
        ...
        return {"wing": "личные_мысли", "room": "inbox", "reason": "автоопределение не удалось"}
...
async def process_note_input(uid: int, raw_text: str, answer_func):
    ...
    try:
        ...
        kb.row(
            ...
            types.InlineKeyboardButton(text="✏️ Другая комната", callback_data="pn_reclass"),
        )
...
async def cb_pn_save(callback: types.CallbackQuery):
    ...
    try:
        ...
        if wing != "личные_мысли" or room != "inbox":
            await mcp.call_tool("mempalace_add_drawer", {
                "wing": "личные_мысли", "room": "inbox", "content": text,
                "added_by": "telegram_bot",
                "source_file": "personal_note"
            })
...
async def cb_pn_reclass(callback: types.CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    kb.row(types.InlineKeyboardButton(text="💭 личные_мысли", callback_data="pn_wing:личные_мысли"))
    for wing, rooms in taxonomy.items():
        if wing != "личные_мысли":
            kb.row(types.InlineKeyboardButton(text=f"🕸️ {wing}", callback_data=f"pn_wing:{wing}"))
    kb.row(types.InlineKeyboardButton(text="❌ Отмена", callback_data="pn_cancel"))

    await callback.message.edit_text("<b>Выберите крыло:</b>", parse_mode="HTML", reply_markup=kb.as_markup())
...
async def cb_pn_wing(callback: types.CallbackQuery):
    ...
    for i, room in enumerate(room_list[:15]):
        kb.row(types.InlineKeyboardButton(text=f"🪪 {room}", callback_data=f"pn_room:{i}"))
    kb.row(types.InlineKeyboardButton(text="🆕 inbox", callback_data="pn_room:-1"))
    kb.row(types.InlineKeyboardButton(text="\u25c0\ufe0f Назад", callback_data="pn_reclass"))
...
async def cb_pn_room(callback: types.CallbackQuery):
    ...
    kb.row(types.InlineKeyboardButton(text="\u25c0\ufe0f К выбору крыла", callback_data="pn_reclass"))
    kb.row(types.InlineKeyboardButton(text="❌ Отмена", callback_data="pn_cancel"))
...
PAGE_SIZE = 5
_list_state: TtlDict[int, dict] = TtlDict()
_active_drawer: TtlDict[int, str] = TtlDict()

@router.message(F.text == "📖 Личные мысли")
@allowed_only
async def cmd_list_personal_notes(message: types.Message):
    await _show_pn_page(message, message.answer, wing="личные_мысли", room="inbox", offset=0)
...
async def _show_pn_page(msg_or_cb, edit_func, wing: str, room: str, offset: int):
    ...
    try:
        ...
        if not drawers:
            ...
            return await edit_func("📖 Пока нет личных заметок.", parse_mode="HTML", reply_markup=kb.as_markup())
        ...
        for i, d in enumerate(drawers):
            kb.row(types.InlineKeyboardButton(
                text=f"📄 {offset + i + 1}",
                callback_data=f"pn_v:{i}"
            ))
        nav_row = []
        if offset > 0:
            nav_row.append(types.InlineKeyboardButton(
                text="\u25c0\ufe0f Назад", callback_data=f"pn_p:{max(0, offset - PAGE_SIZE)}"
            ))
        if offset + PAGE_SIZE < total:
            nav_row.append(types.InlineKeyboardButton(
                text="\u25b6\ufe0f Вперед", callback_data=f"pn_p:{offset + PAGE_SIZE}"
            ))
        if nav_row:
            kb.row(*nav_row)
...
async def cb_pn_view(callback: types.CallbackQuery):
    ...
    try:
        ...
        kb.row(
            ...
            types.InlineKeyboardButton(text="\u25c0\ufe0f К списку", callback_data="pn_bl"),
        )
...
async def cb_pn_read_full(callback: types.CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(parts[-1], parse_mode="HTML", reply_markup=kb.as_markup())
...
async def cb_pn_back_to_list(callback: types.CallbackQuery):
    ...
    state = _list_state.get(uid)
    if state:
        await _show_pn_page(callback, callback.message.edit_text, state["wing"], state["room"], state["offset"])
    else:
        await _show_pn_page(callback, callback.message.edit_text, "личные_мысли", "inbox", 0)
...
async def cb_pn_back_to_view(callback: types.CallbackQuery):
    ...
    if state:
        ...
        await _show_pn_page(callback, callback.message.edit_text, state["wing"], state["room"], offset)
    else:
        await _show_pn_page(callback, callback.message.edit_text, "личные_мысли", "inbox", 0)
...
_quote_waiting: TtlDict[int, dict] = TtlDict()

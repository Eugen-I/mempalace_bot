...
from config import CONFIG_AI_FILE, MODELS_CONFIG_PATH, allowed_callback, allowed_only
from services.ai_engine import invalidate_ai_cache
...
def build_settings_kb(user_id: int):
    ...
    mode_txt = {"text": "📝 Текст", "voice": "🎤 Голос", "both": "📝+🎤 Вместе"}.get(
        vs["mode"], "📝 Текст",
    )
    ...
    kb.row(
        types.InlineKeyboardButton(
            text="✨ Gemini", callback_data="set_ai:gemini-flash-latest",
        ),
    )
    kb.row(
        types.InlineKeyboardButton(text="🏠 Ollama модели", callback_data="list_ollama"),
    )
    kb.row(
        types.InlineKeyboardButton(
            text=f"🔊 Режим: {mode_txt}", callback_data="voice_toggle",
        ),
    )
    kb.row(
    ...
    kb.row(
        types.InlineKeyboardButton(text="🔌 MCP", callback_data="palace_mcp"),
...
async def cb_show_help(callback: types.CallbackQuery):
    ...
    kb.row(
        types.InlineKeyboardButton(text="🔄 Синхронизация", callback_data="cmd:sync"),
    )
    kb.row(
        types.InlineKeyboardButton(
            text="⬅️ Назад к настройкам", callback_data="back_settings",
        ),
    )
...
async def cmd_callback_handler(callback: types.CallbackQuery):
    ...
    if command == "photos":
        ...
        if photos:
            msg = "📸 Фото в базе:\n" + "\n".join(
                f"{i}) {p}" for i, p in enumerate(photos, 1)
            )
        ...
        status_msg = await callback.message.answer(
            "🔍 Запускаю анализ последнего фото...",
        )
        ...
        if not fname:
            return await callback.message.answer(
                "⚠️ Нет активного чата для синхронизации.",
            )
...
async def cmd_settings(message: types.Message):
    ...
    await message.answer(
        f"🤖 Текущая модель: <code>{curr}</code>", reply_markup=kb, parse_mode="HTML",
    )
...
async def cb_voice_toggle(cb: types.CallbackQuery):
    ...
    await cb.message.edit_text(
        f"🤖 Модель: <code>{curr}</code>", reply_markup=kb, parse_mode="HTML",
    )
    await cb.answer()

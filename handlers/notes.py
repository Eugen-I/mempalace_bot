...
from config import INSIGHTS_DIR, NOTES_DIR, RESEARCH_DIR, allowed_callback, allowed_only
...
async def cmd_read_archive(message: types.Message):
    ...
    kb = InlineKeyboardBuilder()
    kb.row(
        types.InlineKeyboardButton(text="Последняя", callback_data=f"read|{prefix}|1"),
    ...
    await message.answer(
        f"📂 Архив: <code>{os.path.basename(target)}</code>",
...
async def cb_read(cb: types.CallbackQuery):
    ...
    if not target:
        return await cb.answer("Папка не найдена")

    files = sorted(
        [f for f in os.listdir(target) if not f.startswith(".")],
        key=lambda x: os.path.getmtime(os.path.join(target, x)),
        reverse=True,
    )[: int(count)]
    if not files:
        return await cb.answer("Пусто")

    for f in files:
        with open(os.path.join(target, f), encoding="utf-8") as fh:
            txt = fh.read()
        dt = datetime.fromtimestamp(os.path.getmtime(os.path.join(target, f))).strftime(
            "%d.%m.%Y %H:%M",
        )

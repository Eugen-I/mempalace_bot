...
NOTES_DIRS = {"!": "my_notes", "!!": "insights", "???": "research"}
...
async def link_note_async(note_path: str, note_content: str, prefix: str = "!"):
    ...
    try:
        ...
        if len(query) < 10:
            logger.debug(
                f"[LINKER] Заметка слишком короткая для связывания: {note_path}",
            )
            return
        ...
        if not raw_results or "Ничего не найдено" in raw_results:
            logger.info(
                f"[LINKER] Нет связанных записей для: {os.path.basename(note_path)}",
            )
            return
        ...
        for line in lines:
            ...
            if match:
                ...
                filename = (
                    os.path.basename(file_info) if "/" in file_info else file_info
                )
        ...
        related_items = [
            item for item in related_items if item["filename"] != own_filename
        ][:6]
        ...
        logger.info(
            f"[LINKER] ✅ Добавлено {len(related_items)} связей в {own_filename}",
        )

...
from config import PDF_ARCHIVE_DIR, PROMPTS_FILE
...
DEFAULT_PROMPTS = {
    "general": {
        "name": "📄 Общий анализ",
        "prompt": "Проанализируй документ. Выдели ключевые идеи, темы и сделай краткое саммари.",
    },
    "medical": {
        "name": "🏥 Медицина / Анализы",
        "prompt": "Проанализируй медицинские данные. Выдели показатели, отклонения, рекомендации.",
    },
    "psychology": {
        "name": "🧠 Психология / Юнг",
        "prompt": "Проанализируй текст через призму аналитической психологии. Найди архетипы, символы, инсайты.",  # noqa: E501
    },
    "literature": {
        "name": "📚 Литература / Книги",
        "prompt": "Суммаризируй текст книги. Выдели сюжет, персонажей, главные мысли.",
    },
    "programming": {
        "name": "💻 Программирование",
        "prompt": "Проанализируй технический текст или код. Объясни логику, найди ошибки.",
    },
    "language": {
        "name": "🌍 Изучение языков",
        "prompt": "Выдели полезные фразы, грамматические конструкции и словарный запас.",
    },
    "sport_diet": {
        "name": "🏃 Спорт / Диетология",
        "prompt": "Проанализируй план тренировок или питания. Выдели нагрузку, БЖУ, рекомендации.",
    },
    "contradiction": {
        "name": "⚖️ Поиск противоречий",
        "prompt": "Найди логические нестыковки, конфликтующие факты или противоречия внутри текста.",  # noqa: E501
    },
    "scientific": {
        "name": "🔬 Научная проверка",
        "prompt": "Проверь утверждения на соответствие научным данным. Укажи спорные моменты.",
    },
}
...
async def analyze_large_document(
    engine: str, model: str, full_text: str, prompt_tpl: str, msg,
) -> str:
    ...
    if chars < CHUNK_MAX_CHARS:
        ...
        try:
            await msg.edit_text(
                f"🧠 <code>{model}</code> анализирует... (~{estimate_tokens(full_text) // 1000}k токенов)",  # noqa: E501
                parse_mode="HTML",
            )
        except Exception:
            pass
        return await get_ai_response_async(
            engine, model, [{"role": "user", "content": full_prompt}],
        )
    ...
    try:
        await msg.edit_text(
            f"📄 Документ большой ({chars // 1000}k символов). Анализирую по частям ({num_chunks} частей)...",  # noqa: E501
            parse_mode="HTML",
        )
    except Exception:
        pass
    ...
    for i, chunk in enumerate(chunks, 1):
        try:
            await msg.edit_text(
                f"📄 Часть {i}/{num_chunks} (~{len(chunk) // 1000}k символов)...",
                parse_mode="HTML",
            )
        except Exception:
            pass
        ...
        answer = await get_ai_response_async(
            engine, model, [{"role": "user", "content": part_prompt}],
        )
        if answer and not answer.startswith("❌"):
            chunk_analyses.append(f"=== ЧАСТЬ {i}/{num_chunks} ===\n{answer}")
        else:
            chunk_analyses.append(
                f"=== ЧАСТЬ {i}/{num_chunks} ===\n[Ошибка анализа этой части]",
            )
            logger.warning(f"Chunk {i}/{num_chunks} analysis failed: {answer}")

    try:
        await msg.edit_text(
            f"🧠 Собираю итоговый анализ из {num_chunks} частей...", parse_mode="HTML",
        )
    except Exception:
        pass

    combine_prompt = PDF_COMBINE_PROMPT.format(chunks="\n\n".join(chunk_analyses))
    final = await get_ai_response_async(
        engine, model, [{"role": "user", "content": combine_prompt}],
    )
